...
print("1:", current_time_kst.strftime('%Y-%m-%d %H:%M:%S'))
...
#     current_time_kst = datetime.now(KST)
#     print("2:", current_time_kst.strftime('%Y-%m-%d %H:%M:%S'))

#     await websocket.send_text("play_warning_message")

#     # play_warning_message 재생 시간(초) 동안 대기
#     warning_message_duration = 15  # 실제 재생 시간에 맞게 설정
#     await asyncio.sleep(warning_message_duration)

#     current_time_kst = datetime.now(KST)
#     print("3:", current_time_kst.strftime('%Y-%m-%d %H:%M:%S'))

#     print("Listening for '도와줘' or '괜찮아'...")
#     timeout_duration = 30  # 타임아웃 시간 (초 단위)
#     recognized = False
...
#     print("10초 동안 응답이 없어 무응답으로 처리합니다.")
#     await websocket.send_text("play_no_response_message")
#     send_line_notify("위험 예상 상황 발생 timeout error - 도움 요청")
#     requests.post(f"{CAM_SERVER}/start")
...
async def handle_abnormal_situation_file(file: UploadFile = File(...)):
    ...
    # 현재 한국 시간 출력
    current_time_kst = datetime.now(KST)
    print("After warning message (KST):", current_time_kst.strftime('%Y-%m-%d %H:%M:%S'))
